...
fn check_tensor(
    inout a: UnsafePointer[Scalar[dtype]], inout b: UnsafePointer[Scalar[dtype]], n: Int, label: StringRef
) -> Bool:
...
fn main() raises:
    ...
    var state_header = UnsafePointer[Scalar[DType.int32]].alloc(256)
    ...
    var x = UnsafePointer[Scalar[dtype_int]]().alloc(B * T)
    var y = UnsafePointer[Scalar[dtype_int]]().alloc(B * T)

    var expected_logits = UnsafePointer[Scalar[dtype]]().alloc(B * T * V)
    var expected_loss = UnsafePointer[Scalar[dtype]]().alloc(1)
