...
def matmul_forward(
    ...
    # V1 variant: the forward GEMM is delegated to MAX's linalg.matmul (CPU,
    # cache/register-tiled, SIMD-vectorized, multi-threaded) instead of the
    # hand-written loop. bias is added in a separate parallelized pass.
    # inp is (B,T,C), weight is (OC,C), bias is (OC); out is (B,T,OC).
    # As a plain GEMM: out(M,OC) = inp(M,C) @ weight(OC,C)^T, M = B*T.
    var M = B * T

    var a = TileTensor(
        Span[Scalar[dtype], ImmutAnyOrigin](
            ptr=inp.as_immutable().as_unsafe_any_origin(), length=M * C
        ),
        row_major(M, C),
    )
    var w = TileTensor(
        Span[Scalar[dtype], ImmutAnyOrigin](
            ptr=weight.as_immutable().as_unsafe_any_origin(), length=OC * C
        ),
        row_major(OC, C),
    )
    var c = TileTensor(
        Span[Scalar[dtype], MutAnyOrigin](
            ptr=vout.as_unsafe_any_origin(), length=M * OC
        ),
        row_major(M, OC),
    )
    matmul[transpose_b=True](c, a, w)

    if bias:
        var bias_ptr = bias.value()

        @parameter
        def _add_bias(b: Int):
            for t in range(T):
                var out_bt: FloatPtr = vout + b * T * OC + t * OC

                def _op[width: Int](o: Int) {out_bt, bias_ptr}:
                    out_bt.store(
                        o,
                        out_bt.load[width=width](o)
                        + bias_ptr.load[width=width](o),
                    )

                vectorize[SIMD_WIDTH, unroll_factor=UNROLL_FACTOR](OC, _op)

        parallelize[_add_bias](B)
...
def matmul_backward(
    ...
    # V2 variant: the two backward GEMMs go through MAX's linalg.matmul.
    # dinp and dweight are zeroed each step by zero_grad and written by exactly
    # one matmul_backward call, so we OVERWRITE them (no accumulation). dbias
    # keeps its hand-written reduction. linalg rejects transpose_a, so dweight
    # materializes dout^T into a scratch buffer first (the reference's CPU path).
    var M = B * T

    var a_dout = TileTensor(
        Span[Scalar[dtype], ImmutAnyOrigin](
            ptr=dout.as_immutable().as_unsafe_any_origin(), length=M * OC
        ),
        row_major(M, OC),
    )

    # --- dinp(M,C) = dout(M,OC) @ weight(OC,C)   (transpose_b=False) ---
    ...
    var c_dinp = TileTensor(
        Span[Scalar[dtype], MutAnyOrigin](
            ptr=dinp.as_unsafe_any_origin(), length=M * C
        ),
        row_major(M, C),
    )
    matmul[transpose_b=False](c_dinp, a_dout, b_weight)

    # --- dweight(OC,C) = dout(M,OC)^T @ inp(M,C) ---
    # linalg rejects transpose_a, so transpose dout into scratch(OC,M) first.
    var scratch = alloc[Scalar[dtype]](OC * M)
    var dout_t = TileTensor(
        Span[Scalar[dtype], MutAnyOrigin](
            ptr=scratch.as_unsafe_any_origin(), length=OC * M
        ),
        row_major(OC, M),
    )
    ...
    perms[0] = 1
    perms[1] = 0
    transpose(dout_t, a_dout, perms)
    perms.free()

    var a_dout_t = TileTensor(
        Span[Scalar[dtype], ImmutAnyOrigin](
            ptr=scratch.as_immutable().as_unsafe_any_origin(), length=OC * M
        ),
        row_major(OC, M),
    )
    var b_inp = TileTensor(
        Span[Scalar[dtype], ImmutAnyOrigin](
            ptr=inp.as_immutable().as_unsafe_any_origin(), length=M * C
        ),
        row_major(M, C),
    )
    var c_dweight = TileTensor(
        Span[Scalar[dtype], MutAnyOrigin](
            ptr=dweight.as_unsafe_any_origin(), length=OC * C
        ),
        row_major(OC, C),
    )
    matmul[transpose_b=False](c_dweight, a_dout_t, b_inp)
    scratch.free()

    # --- dbias(OC) = column-sum of dout  (kept hand-written, overwrite) ---
    if dbias:
        var dbias_ptr = dbias.value()

        @parameter
        def _calc_bias(o: Int):
            var s: FLOAT = 0.0
            for bt in range(M):
                s += dout[bt * OC + o]
            dbias_ptr[o] = s

        parallelize[_calc_bias](OC)
